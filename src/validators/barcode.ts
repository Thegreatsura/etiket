...
export function validateBarcode(text: string, type: string): { valid: boolean; error?: string } {
  switch (type) {
    ...
    case 'code39': {
      ...
      if (!valid) return { valid: false, error: 'Code 39 only accepts 0-9, A-Z, -, ., space, $, /, +, %' }
      return { valid: true }
    }

    case 'code93': {
      const valid = /^[0-9A-Z\-. $/+%]*$/.test(text)
      if (!valid) return { valid: false, error: 'Code 93 only accepts 0-9, A-Z, -, ., space, $, /, +, %' }
      return { valid: true }
    }
    ...
    case 'codabar': {
      if (!/^[0-9\-$:/.+ABCDabcd]*$/.test(text)) {
        return { valid: false, error: 'Codabar only accepts 0-9, -, $, :, /, ., +, and A-D start/stop' }
      }
      return { valid: true }
    }
